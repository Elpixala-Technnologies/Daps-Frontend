...
const PopularCategoryUpdateModal = ({ isPopularCategoryModalOpen, setIsPopularCategoryModalOpen, selectedPopularCategory }) => {
    const {
        register,
        handleSubmit,
        formState: { errors },
    } = useForm();
    const { refetchPopularCategory, } = usePopularCategory()
    const handleCancel = () => {
        setIsPopularCategoryModalOpen(false);
    };
    const {
        _id,
        categories,
    ...
    const onSubmit = async (data) => {
        ...
        const res = await fetch(updatePopularCategoryUrl(_id), {
            method: 'PATCH',
            headers: {
    ...
    return (
        <Modal
            ...
            visible={isPopularCategoryModalOpen}
            onCancel={handleCancel}
            footer={null}
            okButtonProps={{ style: { display: 'none' } }}
        >
            <section>
                <div className="flex flex-col w-full gap-4 mx-auto  bg-slate-50 p-6 rounded">
                    <form
...
export default PopularCategoryUpdateModal;